...
class _HomeState extends State<Home> {
  ...
  void getTasks() {
    completed = [["Example task to be completed", "Date", "Time", "Datetime", "TimeOfDay"]];
    not_completed = [["Example task that has been completed", "Date", "Time", "Datetime", "TimeOfDay"]];
  }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(onPressed: () async {
                          dynamic result = await Navigator.pushNamed(context, "/edit_task", arguments: {"task": not_completed[index][0], "date": not_completed[index][1], "time": not_completed[index][2], "datetime_date": not_completed[index][3], "time_time": not_completed[index][4]});
                          if (result != null)
                          {
                            setState(() {
                              not_completed[index] = [result["task"], result["date"], result["time"], result["datetime_date"], result["time_time"]];
                            });
                          }
        ...
        floatingActionButton: FloatingActionButton(tooltip: "Create a task", onPressed: () async {
          ...
          if (result != null) {
            setState(() {
              not_completed.add([result["task"], result["date"], result["time"], result["datetime_date"], result["time_time"]]);
            });
          }
